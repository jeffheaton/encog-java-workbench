...
public class TrainBasicNetwork {
	public static void performTrain(MLMethod mlMethod) {
		...
		if (dialog.process()) {
			...
			if (method instanceof HopfieldNetwork) {
				...
				ChooseBasicNetworkTrainingMethod choose = new ChooseBasicNetworkTrainingMethod(EncogWorkBench.getInstance().getMainWindow(),(BasicNetwork) method);
				if( choose.process() ) {
	...
	private static void performAnnealing(BasicNetwork method,
		...
		if( dialog.process() ) {
			...
			final Train train = new NeuralSimulatedAnnealing(
					method, score, startTemp, stopTemp, cycles);
			startup(train,dialog.getMaxError().getValue()/100.0);
		}
		
	}

	private static void performGenetic(BasicNetwork method, NeuralDataSet trainingData) {
		...
		if( dialog.process() ) {
			...
			final Train train = new NeuralGeneticAlgorithm(
					method, new RangeRandomizer(-1,1), 
					score, 
					populationSize, 
					mutationPercent, 
					percentToMate);
	...
	private static void performLMA(BasicNetwork method, NeuralDataSet trainingData) {
		InputLMA dialog = new InputLMA();
		
		if( dialog.process() ) {
			boolean useBayse = dialog.getUseBayesian().getValue();
			LevenbergMarquardtTraining train = new LevenbergMarquardtTraining((BasicNetwork) method,
					trainingData);
	...
	private static void performRPROP(BasicNetwork method, NeuralDataSet trainingData) {
	...
	private static void performSCG(BasicNetwork method, NeuralDataSet trainingData) {
