...
public class GenerateJava implements Generate {
	...
	private void generateConst()
	{
		...
		switch( this.trainMethod )
		{
			...
			case Genetic:
				...
				source.append("  public static final int POPULATION_SIZE = 5000;\n");						
				source.append("  public static final double MUTATION_PERCENT = 0.1;\n");  
				source.append("  public static final double MATE_PERCENT = 0.25;\n");
				source.append("  public static final double MAX_ERROR = 0.01;\n");
				break;
	...
	public void generateTraining()
	{
		...
		source.append("  public static BasicNetwork trainNetwork(BasicNetwork network,NeuralDataSet trainingSet) {\n");
		...
		switch( this.trainMethod )
		{
			...
			case Genetic:
				...
				source.append("    final Train train = new TrainingSetNeuralGeneticAlgorithm(\n");
				source.append("      network,\n");
				source.append("      true,\n"); 
				source.append("      trainingSet,\n"); 
			...
			case Anneal:
				...
				source.append("    final Train train = new NeuralSimulatedAnnealing(\n");
				source.append("      network, \n");
				source.append("      trainingSet, \n");
		...
		if( this.trainMethod!=TrainingMethod.TrainHopfield )
		{
			...
			source.append("    do {\n");
			source.append("      train.iteration();\n");
			source.append("      System.out.println(\"Iteration #\" + epoch + \" Error:\" + train.getError());\n");
			source.append("      epoch++;\n");
			source.append("    } while ((epoch < MAX_ITERATION) && (train.getError() > MAX_ERROR));\n");
			source.append("\n");
		}
	...
	public void generateQuery()
	{
		...
		source.append("  public static void queryNetwork(BasicNetwork network,NeuralDataSet trainingSet) {\n");
		...
		source.append("        System.out.print(pair.getInput().getData(i));\n");
		source.append("      }\n");
		...
		source.append("      for(int i=0;i<pair.getIdeal().size();i++) {\n");
		source.append("        if( i!=0 )\n");
		source.append("          System.out.print(\",\");\n");
		source.append("        System.out.print(pair.getIdeal().getData(i));\n");
		source.append("      }\n");
	...
	public void generateMain()
	{
		...
		else 
		{
			...
			addClass(NeuralDataSet.class);
			  source.append("      EncogPersistedCollection encog = new EncogPersistedCollection();\n");
			  source.append("      encog.load(\"");
			  source.append(fixPath(EncogWorkBench.getInstance().getCurrentFileName()));
			  source.append("\");\n");
			  source.append("\n");
			  source.append("      final NeuralDataSet trainingSet = (NeuralDataSet) encog.find(\"");
			  source.append(this.training.getName());
			  source.append("\");\n");
			  source.append("      BasicNetwork network = (BasicNetwork) encog.find(\"");
